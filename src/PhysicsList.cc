...
void PhysicsList::ConstructEM()
{
  ...
  while( (*theParticleIterator)() )
  {
    ...
    if (particleName == "e-") {
      ...
      G4ElectronCapture* ecap = new G4ElectronCapture("Target", 16.7*eV);//16.7*eV);
  ...
  em_config->SetExtraEmModel("e-","eIoni",mod,"Target",0*eV,10*TeV, new G4UniversalFluctuation());
  //it was 0 eV treshhold
  ...
  //G4ElectronCapture* ecap = new G4ElectronCapture("Target", 1*eV);//16.7*eV
  //  pmanager->AddDiscreteProcess(ecap);
  ...
  /**
  msc->SetActivationLowEnergyLimit(100*MeV);
  em_config->SetExtraEmModel("mu-","msc",msc,"Target");

  mod = new G4BraggModel();
  mod->SetActivationHighEnergyLimit(20*eV);
  em_config->SetExtraEmModel("mu-","muIoni",mod,"Target",0.0,2*MeV, new G4IonFluctuations());

  mod = new G4BetheBlochModel();
  mod->SetActivationLowEnergyLimit(0.99*GeV);
  em_config->SetExtraEmModel("mu-","muIoni",mod,"Target",2*MeV,100*GeV, new G4IonFluctuations());
  **/
  // ---> MicroElec processes activated                                                                                                                

  //  mod = new G4MicroElecElasticModel();
  //em_config->SetExtraEmModel("mu-","mu-_G4MicroElecElastic",mod,"Target",0.0,100*MeV);
  /**
  mod = new G4MicroElecInelasticModel();
  em_config->SetExtraEmModel("mu-","mu-_G4MicroElecInelastic",mod,"Target",50* keV,10*GeV);
  **/
